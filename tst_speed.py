...
def main():
    # init train class
    cifar_train = CifarTrain(log_time)
    
    # select train and test device 
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    # load pretrained model
    model = get_pretrained_vgg19().to(device)
    ...
    for i in range(0,11):
        # model is being physicaly pruned so we need to reinit optimizer
        cifar_train.initialize(model, lr=1e-5)
        
        t0 = time.time()
        #cifar_train.train(i, 2500)
        t1 = time.time()
        eval_time_gpu = get_eval_time(model, device)
        
        model = model.cpu()
        eval_time_cpu = get_eval_time(model, 'cpu')
        msglogger.info("Train time {} s  ;  Eval time GPU {} ms and CPU {} ms".format(t1-t0, eval_time_gpu, eval_time_cpu))
